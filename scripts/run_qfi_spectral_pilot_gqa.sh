...
if [ "${MODE}" != "dry" ] && [ "${MODE}" != "full" ]; then
    echo "Usage: bash scripts/run_qfi_spectral_pilot_gqa.sh dry|full" >&2
    exit 2
fi
...
wait_batch() {
    ...
    for index in "${!BATCH_PIDS[@]}"; do
        ...
        if wait "${pid}"; then
            echo "[qfi-spectral] completed ${description}"
        else
            echo "[qfi-spectral] failed ${description}; inspect its log" >&2
        fi
    done
    BATCH_PIDS=()
    BATCH_DESCRIPTIONS=()
...
for profile in "${PROFILE_LIST[@]}"; do
    for keep_num in "${K_LIST[@]}"; do
        ...
        if [ "${FORCE_VALUE}" != "1" ] && is_full_complete "${log_path}"; then
            echo "[qfi-spectral] skip completed profile=${profile} K=${keep_num} log=${log_path}"
            continue
        fi
        ...
        echo "[qfi-spectral] launch mode=${MODE} profile=${profile} K=${keep_num} gpu=${gpu}"
...
echo "[qfi-spectral] all scheduled ${MODE} jobs finished"
